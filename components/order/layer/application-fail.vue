<template>
  ...
  <mt-popup
    ...
    <div class="application-refunded-content">
      <!--编辑栏-->
      <!-- <div class="application-refunded-content-title">你的订单支付时间已经超过90天</div> -->
      ...
      <div class="edit-line bm-box bm-box-horizontal bm-box-vertical-center">
         ...
         <input :value="accountName" type="text" class="bm-flex edit-input" >
      </div>
       <div class="edit-line bm-box bm-box-horizontal bm-box-vertical-center">
         <div class="edit-label">开户行：</div>
        <div @click="selectShow.accountBank = true" class="bm-flex edit-select">
          <span>{{accountBank}}</span> <i class="iconfont icon-down-min"></i>
        </div>
      </div>
       <div class="edit-line bm-box bm-box-horizontal bm-box-vertical-center">
    ...
    <div @click="applicationRefundPopupVisible = false" class="close-btn"><i class="iconfont icon-pay-fail"></i></div>
     <commonSelect
      :show.sync="selectShow.accountBank"
      :options="options.accountBank"
      :type="'accountBank'"
      ...
      return {
        ...
        options: {
          accountBank: [
            {
              flex: 1,
              values: ['招商银行', '兴业银行', '农业银行', '建设银行', '邮政初速', '工商银行'],
              defaultIndex: 0, // 默认值
              className: 'slot-1',
              textAlign: 'center'
            }
